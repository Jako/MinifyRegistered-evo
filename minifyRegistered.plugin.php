 ...
 *
 * @internal    @description: <strong>0.1.1</strong> collect the registered
 ...
 * @internal	@configuration: &groupJs=Group minified files in 'assets/js':;list;yes,no;yes
 * 
			...
			foreach($modx->loadedjscripts as $scriptSrc => $scriptParam) {
				...
				if(strpos($scriptSrc, '<') === FALSE) {
					if(substr(trim($scriptSrc), -3) == '.js') {
						if($groupJs && (trim(dirname(trim($scriptSrc)), '/') == 'assets/js')) {
